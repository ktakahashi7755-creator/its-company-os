...
- **両刀（サーバ×NWの両方の実務）は絶対条件。** 片刀（サーバのみ/NWのみ）は必須軸を0にして excluded へ。ここが精度の生命線。
  ※本案件は事業会社の情シスインフラPL募集。**情シス実務/BPO経験・PL/リーダー(2〜5名)・進捗/品質管理**が必須内訳（欠ければ大幅減点）。
  **セキュリティ(EDR初動等)は本案件では歓迎＝加点**（必須ではない。無くても除外しない）。
- **否定・希薄化表現に注意（プレフィルタのrecall設計を採点で補正）：** プレフィルタは語一致で通すため
  「サーバ実務**なし**」「ネットワーク経験は**無い**」等の否定を含む要員も通過し得る。**その領域の実務が『無い/ごく僅か』**
  （例：ヘルプデスク/端末キッティング**専任**で構築・運用ディレクション実務が確認できない）なら該当スキル未充足＝両刀不成立＝除外。
  ただし**実務はあるが一部が薄いだけ**（例：両刀は成立・PL経験が浅い等）は**除外せず『中』で減点**する。両者を混同しない。
- **除外(score<60)は次のハード条件のみに限定する**：①両刀不成立（サーバ or NWの実務が無い/ごく僅か）／②希望単価が支払上限**92万超**／
...
def _text_has_two_sided(text):
    """テキスト（通過根拠/サマリー等）に サーバ証拠語 と NW証拠語 の両方が含まれるか（＝両刀の明示根拠）。
...
def is_pickup(c):
    ...
    次を**全て**満たす時だけ本命とする（＝確度の生命線）：
      ...
      2) 通過根拠/サマリーに サーバ証拠語 × NW証拠語 が両方明示（＝両刀の実務が文面で裏取れる）。
