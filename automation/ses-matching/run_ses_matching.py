...
- **否定・希薄化表現に注意（プレフィルタのrecall設計を採点で補正）：** プレフィルタは語一致で通すため
  「サーバ実務**なし**」「ネットワーク経験は**無い**」「**軽微**」「**浅い**」等の否定・希薄化を含む要員も通過し得る。
  **採点では、その領域の実務が『無い/浅い』なら該当スキルを満たさない扱い**にし、両刀不成立なら必須0＝除外。
  ヘルプデスク/端末キッティング中心で**サーバ×NWの構築・運用ディレクション実務が薄い**要員は、片刀相当として厳しく採点する。
