...
- **2026-07-22 R8｜案件スコープ改定（NW×Sec → サーバ×NW両刀＋情シス/PL）に精度を追随**：
  ...
  - **測定**：段階①プレフィルタ **29件で precision 1.00・recall 1.00・F1 1.00**（純サーバ/純NWの片刀を確実に足切り、語境界の誤爆ゼロ）。
    本命ピックアップ **20/20**。→ **決定論15ステージ全緑（`--stage all`・exit 0）**。
  - **段階②（LLM帯一致）初回実測＝PR #42 の eval CI（gpt-4o-mini・13件）**：本命(85+)一致 **12/13=0.92**（良好）／帯一致 **8/13=0.62**。
    帯ミスは全て同方向＝**中band（両刀+情シスだが商流2次/PL薄い/年齢超）の過剰除外**。中band保護プロンプトを入れて再計測したが
    **出力バイト単位同一＝gpt-4o-miniの能力上限**（プロンプトでは動かない）。本命層は決定論ガード込みで信頼でき、中bandの改善は
    上位モデル採用（変動費・代表判断）の領域。**eval CIの段階②LLM計測は毎push→手動実行(workflow_dispatch)限定に変更**（コスト最優先）。
  - **gpt-4o で1回計測（代表承認）**：帯一致 **10/13=0.77**（mini比 +0.15）／本命 **13/13=1.00**（満点）。
    S07(年齢58歳)が除外→**flag**に是正（原則回復）、S08(完全両刀本命)が除外→**高**に是正。残ミスはS02/S09/S06の境界判断。
    → 段階②の計測モデルは workflow_dispatch の `scoring_model` 入力で上書き可（例 gpt-4o）。本番採点モデルは代表判断で `vars.OPENAI_MODEL` に設定。
