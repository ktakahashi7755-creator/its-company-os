...
## ✅ 済（2026-07-28）：京楽案件を PR #42 土台＋07-28 JD値で再稼働（サーバ×NW両刀）

- 代表が案件を再送（大手遊技機メーカー 情シス支援・インフラリーダー）→ **7/21停止からの再開トリガー**。
- **PR #42（京楽スコープ改定・サーバ×NW両刀・作り込み済み）を土台に採用**し、以下を07-28 JD値へ更新：
  - **年齢 40歳程度 → 35歳まで**、**単価 〜100万（支払92万以下）→ 80〜90万（支払82万以下）**（案件ファイル・scoring.md・MAIL-BLOCK）。
  - ⚠️ **値の出所差異（要・代表確認）**：07-22版は「確定・100万/40歳」、07-28 JDは「80-90万/35歳」。どちらが最新のクライアント値か代表確認（単価は供給側の有無・粗利に直結）。案件ファイル §5 に明記。
- スコープは **サーバ×NW両刀**（セキュリティは歓迎・横断）＝PR #42の精度設計を踏襲。両刀ゲート `PREFILTER_GROUPS`＝サーバ群×NW群。
- **鮮度厳重＝配信3日以内**（`FRESH_DAYS=3`・代表指示2026-07-28）。本命は PICKUP_MIN=90、採点 gpt-4o、IMAPハング対策（timeout25分/MAX_FETCH500）はPR #42のまま。
- eval 決定論15ステージ全緑（exit 0）。日次自動（毎朝7:00 JST狙い）＋一時トリガーで本日分を即実行。
- **経緯メモ**：本セッションは7/13の古い文脈から再開したため、当初この案件を知らずに二重実装しかけた（NW×Sec・35歳/80-90万）。PR #42 の存在に気づき、PR #42を正として一本化・重複を巻き戻した。旧 `案件_20260728_遊技機情シスインフラLead.md` は不採用（PR #42の `案件_20260711_遊技機NWSec.md`＝サーバ×NW情シスPL版に統合）。
