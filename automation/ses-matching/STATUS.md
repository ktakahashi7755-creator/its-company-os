...
## ✅ 済（2026-07-31）：送信後フォローアップ下書きエンジン（クロージング支援・drafts only）

戦略 `docs/cache-automation-strategy.md` §6-② の実装。**下書き供給までで止まっていた「送信後」の空白**（面談化の一歩手前）を自動化。

- **新エンジン `followup.py`**：Notion「SES提案トラッカー」の **ステータス=送信済** の行を唯一の真実源とし、
  各行の `last_edited_time`（＝代表が『送信済』にした時刻）を送信日の代理値に、**営業日経過（既定3営業日・土日除外）**を算出。
  しきい値以上 音沙汰が無い提案へ、**丁寧なリマインド返信の下書き**＋**クロージング支援メモ（面談日程調整の定型文＋単価交渉の想定問答）**を
  sales@ の下書き(Drafts)へ IMAP APPEND で保存。**送信は一切しない**（\\Draft のみ）。
- **重複防止**：リマインドの `X-ITS-Key` は原提案キー＋`|FU{n}`＝原提案の下書きと衝突せず、Drafts検索で実行跨ぎの二重作成を防ぐ。既定は1提案1通（`FOLLOWUP_MAX`）。
- **ガードレール**：`run_ses_matching.validate_draft` を再利用（From＝sales@固定・REOorGA混入禁止・ITS識別必須）。
  **単価交渉の想定問答は実額があれば使い、無ければ「要確認」**（捏造しない）。宛先未確定は注記付きで作る（誤送信より安全側）。
- **新テンプレ** `template-リマインド.txt`。**オフライン検証** `examples/sample-followup.json`（個人情報なしダミー）で決定論に完走。
- **eval**：決定論ステージ **`followup`（24/24）** 追加＝営業日計算・UTC→JST境界・しきい値判定・重複防止・
  ガードレール（REOorGA拒否/宛先未確定）・単価捏造なし・Notion行の正規化を回帰。**全18ステージ緑（`--stage all`・exit 0）**。
- **ワークフロー** `.github/workflows/ses-followup.yml`：eval gate＋毎朝8:00 JST狙い（cron `0 23 * * *` UTC）。
  `SALES_IMAP_*` / `NOTION_*` 未登録の間は自動でスキップ（＝安全）。手動 dispatch＋`offline=true` で文面確認可。
- **前提（代表運用）**：トラッカーで送った提案を `未送信→送信済` に変えると、その時刻が送信日になりリマインド起点になる（＝既存の運用がそのまま燃料）。
- **限界（正直に）**：リマインド下書きを代表が送信して Drafts から消すと、Drafts検索での重複判定が効かず稀に2通目が出る余地。既定 `FOLLOWUP_MAX=1` で上限を1に固定して抑制。
...
- **年齢：36歳以上は選定しない（ハード除外）**。`AGE_HARD_LIMIT=35`（既定無効・代表明示の案件のみ有効）。
  `reconcile_scores` の決定論ゲート＝確実に36歳以上(36歳/40代/50代)はスキル不問で excluded、跨ぐ/不明(30代/不明)は本命保留＋「年齢要確認」。eval `agegate`(10/10) 追加＝16ステージ全緑。
- **単価：クライアント支払100〜120万・要員希望80〜90万を最優先**（粗利¥10〜30万）。scoring.md・案件§5に反映（先の「クライアント上限80-90万」誤りを訂正）。
- **本番 run #36（2026-07-29分）の結果**：Notion日次ページ作成。**自動下書き0件**＝直近3日の該当要員が全員36歳以上（K.N 39歳・52歳・53歳・57歳等）で年齢ハード除外、または両刀/90点に届かず。**0件はバグではなく仕様どおりの結果**。
  - ⚠️ **歩留り注意（代表判断事項）**：35歳ハード上限＋鮮度3日＋本命90点＋サーバ×NW両刀は非常に厳しく、日によって本命0件になり得る。母数を増やすなら 鮮度(3→5日)／PICKUP_MIN(90→85)／年齢運用 のいずれかを緩める選択肢（いずれも代表判断）。
