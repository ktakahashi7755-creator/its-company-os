...
## 🛑 現在の状態（2026-07-21〜）＝**一時停止中**（旧・単一案件エンジン）

- **KNさんがオファー獲得** 🎉（遊技機NW/Sec案件は充足）。→ 本案件のマッチング・自動下書きを**停止**。
- **案件スコープ見直し**が入ったため、**案件マッチングを全停止**。代表から**案件を再送後に集め直す**。
- 実施した停止措置：
  1. **両ワークフローの自動実行を停止**（`ses-matching.yml` / `ses-matching-poll.yml` の `schedule:` をコメントアウト。`workflow_dispatch` は残置）。
  2. **アクティブ案件をアーカイブ退避**：`案件_20260711_遊技機NWSec.md` → `archive/` へ移動（`load_case_defs()` はトップ階層のみ走査＝安全に無効化）。
     → 手動dispatchしても**アクティブ案件ゼロ＝マッチ0・下書き0**（検証済み）。
  3. eval を案件アーカイブに依存しないよう修正（`finalize`/`drafts` は固定フィクスチャの案件本文で回す）。→ 決定論15ステージ緑（exit 0）。
- **再開手順（案件再送後）**：
...
- **本命の確度を上げる決定論ガード（precision）**：`is_pickup()` が本命の唯一の門。
  ...
  2. `reason`／`summary` に **NW証拠語 × Sec証拠語 が両方**（＝両刀が文面で裏取れる）。揃わなければ本命から外し「両刀根拠の明示要確認（自動下書き保留）」flag。
...
| 項目 | 値 |
|---|---|
| アクティブ案件 | `案件_20260711_遊技機NWSec.md`（🛑 archive/で停止中・再開は代表判断）。正式版rev.2026-07-22b＝**情シスインフラPL／サーバ×NW両刀必須・情シス目線＋PL(2〜5名)必須・セキュリティは歓迎／8月開始・〜100万(支払92万以下)・40歳程度まで・恵比寿・週2出社**。旧NW×Sec・〜80万・45歳上限は失効 |
